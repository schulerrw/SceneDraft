...
'#-hemisphere,C, Cx,Cy,Cz, Px,Py,Pz, N, <N2>
...
' 7 | Color | Cx | Cy | Cz | Px | Py | Pz | StepsLat | StepsLon
  Dim Cx, Cy, Cz As Double ' center point
  Dim ax, ay, az As Double ' direction=axis of rotation
  ...
  ay = CDbl(Trim(B(6)))
  az = CDbl(Trim(B(7)))
  stepsLat = CInt(Trim(B(8)))
  R2 = global_radius
  N2 = global_steps
  If PPPL > 8 Then
     If Len(B(9)) > 0 Then
       N2 = CInt(Trim(B(9)))  'optional overide of #longitudes
     End If
  End If
 ...
 ''' compute base radius R1
 R1 = Sqr(aa * aa + bb * bb + cc * cc)
